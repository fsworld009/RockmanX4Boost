...
Version="ROCKMANX4 BOOST (JP) Ver 2016.08.14"
...
function timer_2(addr)
	...
	timer2word=memory.read_u16_le(addr+0x38);
	if (timer2word > 0x000F) then
			memory.write_u16_le(addr+0x38,0x01);
	end
end
...
if boost==true then
	...
	for i = 0,12 do
		--3013BFF4 0001
		case=0 --default
		--enemyid=memory.read_u32_le(0x13BFE0);
		enemyid=memory.read_u32_le(addr-0x14);
		...
		if case==0 then --default
			--addr=0x13BFF4;
			timer(addr)
			--80104D9C大型機甲兵 --enemyid=memory.read_u32_le(0x13BFE0);
			...
			if enemyid==0x800FBC88 then
				if(memory.read_u16_le(addr-0x22)==0x0001) then --horizontal speed? 0x13BFD2 C06E C10A C1A6 C242
					memory.write_u16_le(addr-0x22,0x0005) --horizontal speed?
				elseif(memory.read_u16_le(addr-0x22)==0xFFFF) then --horizontal speed?
					memory.write_u16_le(addr-0x22,0xFFFB) --horizontal speed?
				end
			end
			--小蝙蝠 0x800FDE44
			if enemyid==0x800FDE44 then
				if(memory.read_u16_le(addr-0x22)==0x0003) then --horizontal speed?
					memory.write_u16_le(addr-0x22,0x0009) --horizontal speed?
				elseif(memory.read_u16_le(addr-0x22)==0xFFFD) then --horizontal speed?
					memory.write_u16_le(addr-0x22,0xFFF7) --horizontal speed?
				end
			end
		end
		...
		if case==1 then --sigma --13C02C 雷射炮次數 5 4 3 2 1 0 13C045:雷射炮on off
			...
			if headtype<6 and headtype~=8 then --只要不是大頭就用原本的timer
				...
				--timer_2(addr)
				timer_3(addr)
				timer2word=memory.read_u16_le(0x13C02C);
				if (timer2word > 0x000F) then
					memory.write_u16_le(0x13C02C,memory.read_u16_le(0x13C02C)-1);
				end
			end
		...
		if case==6 then --frost walrus
			...
			timer_1(addr);
			timer_3(addr);
			if(emu.framecount()%120<30) then
			memory.writebyte(0x13C038,0x8E)
			elseif(emu.framecount()%120<60) then
			memory.writebyte(0x13C038,0x92)
			elseif(emu.framecount()%120<90) then
			...
			if(memory.read_u16_le(addr-0x22)>0xFF00) then --horizontal speed?
				memory.write_u16_le(addr-0x22,0xFFFC) --horizontal speed?
			elseif(memory.read_u16_le(addr-0x22)>0) then --horizontal speed?
				memory.write_u16_le(addr-0x22,0x0004) --horizontal speed?
			end
		end
		if case==7 then --Jet Stingray
			...
			if(memory.readbyte(0x13C034)==0xE2) then --上下衝
				if(memory.read_u16_le(0x13BFD6)==0xFFF7) then --Vertical speed?
					memory.write_u16_le(0x13BFD6,-speed) --Vertical speed?
				elseif(memory.read_u16_le(0x13BFD6)==0x0009) then --Vertical speed?
					memory.write_u16_le(0x13BFD6,speed) --Vertical speed?
				end
			end
			--橫衝
				if(memory.read_u16_le(0x13BFD2)==0x0006) then --horizontal speed?
					memory.write_u16_le(0x13BFD2,speed) --horizontal speed?
				elseif(memory.read_u16_le(0x13BFD2)==0xFFFA) then --horizontal speed?
					memory.write_u16_le(0x13BFD2,-speed) --horizontal speed?
				end
			--拋物線
			if(memory.readbyte(0x13BFDD)==0xBE) then
				if(emu.framecount()%8==0) then
					memory.write_u16_le(0x13BFD6,memory.read_u16_le(0x13BFD6)+1);--Vertical speed?
				end
				if(memory.read_u16_le(0x13BFD2)==0x0005) then --horizontal speed?
					memory.write_u16_le(0x13BFD2,speed) --horizontal speed?
				elseif(memory.read_u16_le(0x13BFD2)==0xFFFB) then --horizontal speed?
					memory.write_u16_le(0x13BFD2,-speed) --horizontal speed?
				end
			elseif(memory.readbyte(0x13BFDD)==0x30) then
				if(memory.read_u16_le(0x13BFD6)==0xFFFE) then --Vertical speed?
					memory.write_u16_le(0x13BFD6,0xFFEF) --Vertical speed?
				end
			end
		...
		if case==197 then --Intro Stage Boss
			timer(addr);
			if(memory.read_u16_le(0x13BFD2)==0x0008) then --horizontal speed?
				memory.write_u16_le(0x13BFD2,0x0010) --horizontal speed?
				memory.write_u16_le(0x13BFD6,0xFFFE) --vertical speed?
			elseif(memory.read_u16_le(0x13BFD2)==0xFFF7) then --horizontal speed?
				memory.write_u16_le(0x13BFD2,0xFFF0) --horizontal speed?
				memory.write_u16_le(0x13BFD6,0xFFFE) --vertical speed?
			end
			if(memory.read_u16_le(0x13BFD2)==0x0001) then --horizontal speed?
				memory.write_u16_le(0x13BFD2,0x0008) --horizontal speed?
			elseif(memory.read_u16_le(0x13BFD2)==0xFFFF) then --horizontal speed?
				memory.write_u16_le(0x13BFD2,0xFFF7) --horizontal speed?
			end
		end
		if case==198 then --mid boss of slash beast
			...
			spike_addr=0x13F42A;
			for j = 0,2 do
				if     memory.read_u16_le(spike_addr)==0xFFFC then memory.write_u16_le(spike_addr,0xFFFA);
				elseif memory.read_u16_le(spike_addr)==0x0006 then memory.write_u16_le(spike_addr,0x0009);
				elseif memory.read_u16_le(spike_addr)==0xFFFE then memory.write_u16_le(spike_addr,0xFFFD);
				end
		...
		if case==199 then --mid boss in snow base
			...
			if(flag) then
				...
				if(memory.read_u16_le(0x13BFD2)>0xF000) then --horizontal speed?
					memory.write_u16_le(0x13BFD2,0xFFFA) --horizontal speed?
				end
			end
		end
		if case==200 then --mid boss in bio lab
			...
			--enemyid=memory.read_u32_le(0x13BFE0);
			if(memory.read_u16_le(addr-0x1E)==0x0004) then
				memory.write_u16_le(addr-0x1E,0x0007) --VSpeed
			elseif(memory.read_u16_le(addr-0x1E)==0x0006) then
				memory.write_u16_le(addr-0x1E,0x0000) --VSpeed
			end
			if(memory.read_u16_le(addr-0x22)==0x0004) then
				memory.write_u16_le(addr-0x22,0x0008) --HSpeed
			elseif(memory.read_u16_le(addr-0x22)==0x0007) then
				memory.write_u16_le(addr-0x22,0x0001) --HSpeed
			elseif(memory.read_u16_le(addr-0x22)==0xFFFC) then
				memory.write_u16_le(addr-0x22,0xFFF8) --HSpeed
			elseif(memory.read_u16_le(addr-0x22)==0xFFF9) then
				memory.write_u16_le(addr-0x22,0xFFFF) --HSpeed
			end
		end
